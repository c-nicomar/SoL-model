function SkyDiffuse = irradianceDiffuseKing(surfaceTilt, DHI, GHI, apparentSunZenith)
% IRRADIANCE_DIFFUSE_KING Determine diffuse irradiance from the sky on a tilted surface using the King model
%
% Syntax
%   SkyDiffuse = irradianceDiffuseKing(SurfTilt, DHI, GHI, SunZen)
%
% Description
...
p.parse(surfaceTilt, DHI, GHI, apparentSunZenith);

SkyDiffuse = DHI.*(1+cosd(surfaceTilt))./2 + GHI.*(.012*apparentSunZenith-.04).*(1-cosd(surfaceTilt))./2;
