function Location = makelocationstruct(latitude, longitude, varargin)
% makelocationstruct Create a struct to define a site location
%
% Syntax
%   Location = makelocationstruct(latitude, longitude)
%   Location = makelocationstruct(latitude, longitude, elevation)
%
% Description
%   Creates a location struct for use with some PVLib functions. Site
%   information includes latitude, longitude, and optionally elevation.
%
...
%   Input elevation is optional, and should be given in meters above sea
%   level.
%   All inputs must be scalars.
%
%   Output is a struct, Location, consisting of Location.latitude,
%   Location.longitude, and Location.elevation (if elevation provided). 
...
p.addOptional('elevation', 1.9321E100, @(x) all(isscalar(x) & isnumeric(x)));
...
defaultchecker = {'elevation'};
if ~any(strcmp(defaultchecker,p.UsingDefaults))
    Location.elevation = p.Results.elevation;
end
