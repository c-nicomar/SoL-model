function SkyDiffuse = irradianceDiffuseIsotropic(surfaceTilt, DHI)
...
p = inputParser;
p.addRequired('surfaceTilt');%, @(x) all(isnumeric(x) & x<=180 & x>=0 & isvector(x)));
p.addRequired('DHI')%, @(x) all(isnumeric(x) & isvector(x) & x>=0));
p.parse(surfaceTilt, DHI);

SkyDiffuse = DHI *(1+ cosd(surfaceTilt)) * 0.5;
