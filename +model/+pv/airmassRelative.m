function AM = airmassRelative(zenithAngle,varargin)
% airmassRelative    Gives the relative (not pressure-corrected) airmass
%
% Syntax
...
switch(lower(model))
    ...
    case('kastenyoung1989')
        AM = 1./(cosd(zenithAngle)+0.50572.*((6.07995+(90-zenithAngle)).^-1.6364));
    case('kasten1966')
        AM = 1./(cosd(zenithAngle)+0.15.*(93.885-zenithAngle).^-1.253);
    case('simple')
        AM = secd(zenithAngle);
    case('pickering2002')
        AM = 1./(sind(90-zenithAngle+244./(165+47.*(90-zenithAngle).^1.1)));
    case('youngirvine1967')
        AM = 1./cosd(zenithAngle).*(1-0.0012.*(((secd(zenithAngle)).^2)-1));
    case('young1994')
        AM = (1.002432.*(cosd(zenithAngle)).^2 + 0.148386 .* cosd(zenithAngle) + 0.0096467) ./ ...
            (cosd(zenithAngle).^3 + 0.149864 .* cosd(zenithAngle).^2 + 0.0102963 .* cosd(zenithAngle) + 0.000303978);
    case('gueymard1993')
        AM = 1./(cosd(zenithAngle) + 0.00176759 .* zenithAngle .* (94.37515-zenithAngle).^-1.21563);
    otherwise
