function SkyDiffuse = irradianceDiffuseKlucher(surfaceTilt, surfaceAzimuth, DHI, GHI, apparentSunZenith, sunAzimuth)
...
p.parse(surfaceTilt, surfaceAzimuth, DHI, GHI, apparentSunZenith, sunAzimuth);
...
COSTT = cosd(surfaceTilt).*cosd(apparentSunZenith) + sind(surfaceTilt).*sind(apparentSunZenith).*cosd(sunAzimuth-surfaceAzimuth);

F = 1 - ((DHI./GHI).^2);
SkyDiffuse = DHI .* (0.5 .* (1 + cosd(surfaceTilt))) .* (1 + F .* ((sind(surfaceTilt ./ 2)).^3)) .* (1 + F .* ((COSTT).^2) .* ((sind(apparentSunZenith)).^3));
