function [ClearSkyGHI, ClearSkyDNI, ClearSkyDHI]= irradianceIneichenSimplified(time, location, varargin)
...
ClearSkyGHI = cg1.*I0.*cosApparentSunZenith.*exp(-cg2.*AMabsolute.*(fh1+fh2.*(TL-ones(size(TL))))).*exp(0.01.*(AMabsolute).^(1.8));
