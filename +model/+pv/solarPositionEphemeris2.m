function [SunAz,SunAzi,SunEl, ApparentSunEl, SolarTime,Declinacion,HrAngleR,HrAngleR2,RtAscen,RtAscen2,EcLonR,LatR,DecR,SunAzP2] = solarPositionEphemeris2(Time, Location, varargin)
% solarPositionEphemeris Calculates the position of the sun given time, location, and optionally pressure and temperature
%
% Syntax
%   [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location)
%   [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location, Pressure)
%   [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location, Pressure, Temperature)
%   [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location, 'temperature', Temperature)
%
% Description  
%  [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location)
...
%      is unknown but site elevation is known, a conversion function may be
%      used.
%   [SunAz, SunEl, ApparentSunEl, SolarTime]=solarPositionEphemeris(Time, Location, Pressure, Temperature)
...
%   Location.elevation = an optional component of the Location struct, not
%     used in the ephemeris code directly, but it may be used to calculate
%     standard site pressure (see alt2pres function)
% 
% Output Parameters:
...
%
% See also maketimestruct makelocationstruct alt2pres
%          solarIncidentAngle solarPositionSPA
...
Longitude = p.Results.Location.longitude;

%Longitude = table2array(LongitudeT);

% yo Longitude = Location.longitude;
% Ojo ue hay que investigar como cambiar el signo en la tabla Longitude=-1*Longitude1;
...
DayOfYear = date2doy(Year, Month, Day);
...
T1 = EpochDate / 36525;
ObliquityR = DegtoRad * (23.452294 - 0.0130125 * T1 - 0.00000164 * T1.^2 + 0.000000503 * T1.^3);
MlPerigee = 281.22083 + 0.0000470684 * EpochDate + 0.000453 * T1 .^ 2 + 0.000003 * T1 .^ 3;
MeanAnom = mod((358.47583 + 0.985600267 * EpochDate - 0.00015 * T1 .^ 2 - 0.000003 * T1 .^ 3), 360);
...
RtAscen2=linspace(-78.4262, (78.4262+180),8760)'; % yo; no es exacto pero no tiene discontinuidad

HrAngle = LocAST - RtAscen ;
HrAngle = HrAngle - (360 .* sign(HrAngle) .* (abs(HrAngle) > 180));
HrAngleR = DegtoRad .* HrAngle ; 

HrAngle2 = LocAST - RtAscen2 ;
%HrAngle2 = HrAngle2 - (360 .* sign(HrAngle2) .* (abs(HrAngle2) > 180));
HrAngleR2 = DegtoRad .* HrAngle2 ; 

SunAzi = RadtoDeg .* atan2(-1 * sin(HrAngleR), cos(LatR) .* tan(DecR) - sin(LatR) .* cos(HrAngleR));
SunAzP1=-1 * sin(HrAngleR), cos(LatR) .* tan(DecR); % ok
SunAzP2=-cos(LatR) .* tan(DecR) - sin(LatR) .* cos(HrAngleR); % ok

%atan2(SunAzP1,SunAzP2); discontinuity that exists at SunAzP1=0 for all SunAzP2<0.

SunAz = SunAzi + (SunAzi < 0) * 360; %shift from range of [-180,180] to [0,360]
