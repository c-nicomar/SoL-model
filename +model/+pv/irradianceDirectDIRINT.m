function dirintDNI = irradianceDirectDIRINT(GHI, zenithAngle, doy, pressure, varargin)
% IRRADIANCE_DIRECT_DIRINT Determine DNI from GHI using the DIRINT modification of the DISC model
%
% Syntax
%   dirintDNI = irradianceDirectDIRINT(GHI, Z, doy, pressure)
%   dirintDNI = irradianceDirectDIRINT(GHI, Z, doy, pressure, UseDelKtPrime)
%   dirintDNI = irradianceDirectDIRINT(GHI, Z, doy, pressure, UseDelKtPrime, DewPtTemp)
%
% Description
...
maxvectorsize = max([numel(GHI) numel(zenithAngle) numel(doy) numel(pressure)]);
...
p.parse(GHI,zenithAngle,doy, pressure, varargin{:});

GHI = p.Results.GHI(:);
zenithAngle=p.Results.Z(:);
doy=p.Results.doy(:);
...
[DiscDNI, Kt] = irradianceDirectDISC(GHI, zenithAngle, doy, pressure);
...
AM = 1./(cosd(zenithAngle) + 0.15.*((93.885-zenithAngle).^(-1.253))) .* pressure./ 101325;
...
if any(KtPrime > 1)
    error('Error in irradianceDirectDIRINT, KtPrime values above 1 are not allowed.')
end
...
ZBin(zenithAngle>=0 & zenithAngle<25) = 1;
ZBin(zenithAngle>=25 & zenithAngle<40) = 2;
ZBin(zenithAngle>=40 & zenithAngle<55) = 3;
ZBin(zenithAngle>=55 & zenithAngle<70) = 4;
ZBin(zenithAngle>=70 & zenithAngle<80) = 5;
...
ZBin(zenithAngle>=80 & zenithAngle<=180) = 6;
...
if any(DelKtPrime > 1)
    error('Error in irradianceDirectDIRINT, DelKtPrime values above 1 are not allowed.')
end
