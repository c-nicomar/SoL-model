function diffuseSkyIrradiance = irradianceDiffuseHayDavies(surfaceTilt, surfaceAzimuth, diffuseHorizontalIrradiance, directNormalIrradiance, extraterrestrialNormalIrradiance, solarZenith, solarAzimuth)
...
p.parse(surfaceTilt, surfaceAzimuth, diffuseHorizontalIrradiance, directNormalIrradiance, extraterrestrialNormalIrradiance, solarZenith, solarAzimuth);
...
COSTT = cosd(surfaceTilt).*cosd(solarZenith) + sind(surfaceTilt).* sind(solarZenith).*cosd(solarAzimuth-surfaceAzimuth); 
...
RB = max(COSTT,0)./max(cosd(solarZenith),0.01745);
% AI is the Anisotropy index which represents the transmittance through
% atmosphere for beam radiation
AI = directNormalIrradiance./extraterrestrialNormalIrradiance;
...
diffuseSkyIrradiance = diffuseHorizontalIrradiance .* (AI .* RB + (1-AI) .* 0.5 .* (1 + cosd(surfaceTilt)));
diffuseSkyIrradiance = diffuseSkyIrradiance(:);
