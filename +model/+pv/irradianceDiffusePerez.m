function SkyDiffuse = irradianceDiffusePerez(SurfTilt, SurfAz, DHI, DNI, HExtra, SunZen, SunAz, AM, varargin)
% IRRADIANCE_DIFFUSE_PEREZ Determine diffuse irradiance from the sky on a tilted surface using one of the Perez models
%
% Syntax
%   SkyDiffuse = irradianceDiffusePerez(SurfTilt, SurfAz, DHI, DNI, HExtra, SunZen, SunAz, AM)
%   SkyDiffuse = irradianceDiffusePerez(SurfTilt, SurfAz, DHI, DNI, HExtra, SunZen, SunAz, AM, model)
%
% Description
...
%%Ojo que he modificado las siguientes condiciones para que me deje leer NaN
...
%VectorSizes = [numel(SurfTilt), numel(SurfAz), numel(DHI), numel(DNI), ...
 %   numel(SunZen), numel(SunAz), numel(AM)];
%AQUI 
...
%z = zStatic(SunZen); %NOTE: Removal of static call
z = SunZen*pi/180;
%econ = econStatic(z);  %NOTE: Removal of static call
econ = 1.041.*z.^3;

e = zeros(length(DHI),1);
%e(Dhfilter) = ((DHI(Dhfilter) + DNI(Dhfilter))./DHI(Dhfilter) + econ(Dhfilter))./(1 + econ(Dhfilter));
e(Dhfilter) = (1 + (DNI(Dhfilter)./DHI(Dhfilter)) + econ(Dhfilter))./(1 + econ(Dhfilter));
...
%cossun = cossunStatic(SunZen);  %NOTE: Removal of static call
cossun = cosd(SunZen);
%sinsun = sinsunStatic(SunZen);  %NOTE: Removal of static call
sinsun = sind(SunZen);

A = cosd(SurfTilt).*cossun + sind(SurfTilt).*sinsun.*cosd(SunAz-SurfAz);
A(A < 0) = 0;
B = cossun;
B(B < cosd(85)) = cosd(85);
...
% Dec 2012: A bug was identified by Rob Andrews (Queens University) in this equation in PV_LIB
% Version 1.0.  Fixed in Version 1.1.
SkyDiffuse(ebinfilter) = DHI(ebinfilter).* (0.5.* (1-F1(ebinfilter)).*(1+cosd(SurfTilt(ebinfilter))) + F1(ebinfilter) .* A(ebinfilter)./ B(ebinfilter) + F2(ebinfilter).* sind(SurfTilt(ebinfilter)));

% DEBUG Outputs ===========================================================

% skyFull = SkyDiffuse(ebinfilter);
% DHIFull = DHI(ebinfilter);
% isotropicFull = DHI(ebinfilter).* (0.5.* (1-F1(ebinfilter)).*(1+cosd(SurfTilt(ebinfilter))));
% circumsolarFull = DHI(ebinfilter).* (F1(ebinfilter) .* A(ebinfilter)./ B(ebinfilter));
% horizonFull = DHI(ebinfilter).* (F2(ebinfilter).* sind(SurfTilt(ebinfilter)));
% actualIsotropicFull = DHI(ebinfilter) .* (0.5 .*(1 + cosd(SurfTilt(ebinfilter))));

% util.putvar(skyFull);
% util.putvar(DHIFull);
% util.putvar(isotropicFull);
% util.putvar(circumsolarFull);
% util.putvar(horizonFull);
% util.putvar(actualIsotropicFull);
...
function sinsun = sinsunStatic(SunZen)
	persistent sinsunInit
	if isempty(sinsunInit)
        sinsunInit = sind(SunZen);
	end
    
	sinsun = sinsunInit;
end

function cossun = cossunStatic(SunZen)
	persistent cossunInit
	if isempty(cossunInit)
        cossunInit = cosd(SunZen);
	end
