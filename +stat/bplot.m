% function forLegend = stat.bplot(X,varargin)
...
% 
%     stat.bplot(D) will create a boxplot of data D, no fuss.
% 
% T = stat.bplot(D)  If X is a matrix, there is one box per column; if X is a
...
% 
% T = stat.bplot(D,x) will plot the boxplot of data D above the 'x' value x
% 
% T = stat.bplot(D,y,'horiz') will plot a horizontal boxplot at the 'y' value of y
% 
% T = stat.bplot(...,'Property', . . . )
% T = stat.bplot(...,'PropertyName',PropertyValue, . . . )
% 
%   SINGLE PARAMETERS:
% 
...
% 
% stat.bplot(randn(1,30))
% stat.bplot(randn(1,30),'color','black');
% ----
% X = round(randn(30,4)*5)/5; % random, with some duplicates
% T = stat.bplot(X,'points');
...
if size(x,1)>1 && size(x,2)>1 % great, you want to plot a bunch.
    if isempty(varargin)
        forLegend = stat.bplot(x(:,1),1);
        for ii=2:size(x,2)
            hold on;
            stat.bplot(x(:,ii),ii,'nolegend');
        end
    else
        if ~ischar(varargin{1})
            warning('You can''t specify a location for multiple guys, this will probably crash');
        end
        forLegend = stat.bplot(x(:,1),1,varargin{:});
        for ii=2:size(x,2)
            hold on;
            stat.bplot(x(:,ii),ii,'nolegend',varargin{:});
...
% end main stat.bplot function over
...
%% This is the function for calculating the quantiles for the stat.bplot.
